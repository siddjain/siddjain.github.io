...
Let stem:[|\hat{m} + \rangle = \textbf{r}_1] and stem:[|\hat{m} - \rangle = \textbf{r}_2]. Then stem:[\alpha] is nothing but
stem:[r_1^\dagger q_1]. Let's calculate it:

[stem]
++++
\begin{split}
\alpha & = r_1^\dagger q_1 = \frac{1}{\sqrt{2 (1 + m_z)}} \begin{pmatrix} 1 + m_z & m_x - i m_y \end{pmatrix} 
...
Note to self: I think in the https://inst.eecs.berkeley.edu/~cs191/fa10/notes/spins.pdf[lecture note] he is trying to derive the fact that measuring spin along stem:[\hat{n}]
amounts to expressing the wavefunction in the eigenbasis formed by the eigenvectors of stem:[S(\hat{n})] whereas I have always taken it as an a-priori assumption or rule.
He is trying to explain from where that assumption came from. 
