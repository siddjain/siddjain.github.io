...
If stem:[H] is not a function of time, then the solution to the equation is simply:
...
where stem:[t] is removed. Think of it as a tiny constant like the reciprocal of the clock speed of the computer.
So to apply different gates to the system we need to manipulate its Hamiltonian. Quantum computer allows us to program and control the Hamiltonian with time.

Another analysis I did which did not lead me anywhere was as follows: to a first order approximation, assuming stem:[H] is not a function of time, the Schrodinger equation can
then be simplified to (considering an infinitestimal step in time):

[stem]
++++
\begin{equation}
i \hbar (\Psi_2 - \Psi_1) = H \Psi_1
\end{equation}
++++

or, stem:[\Psi_2] is simply:
...
and by this logic the gate is given by stem:[I - i H]. Compare to stem:[\exp (- i H )]. To first order approximation, stem:[\exp (- i H )] is indeed
stem:[I - i H].
